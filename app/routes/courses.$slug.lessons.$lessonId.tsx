...
export default function LessonViewer({ loaderData }: Route.ComponentProps) {
  ...
  const displayBookmarked = pendingBookmark ? !isBookmarked : isBookmarked;

  useEffect(() => {
    if (bookmarkFetcher.state === "idle" && bookmarkFetcher.data?.success) {
      if (bookmarkFetcher.data.bookmarked) {
