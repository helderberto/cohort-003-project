...
describe("commentService", () => {
  ...
  describe("createComment", () => {
    it("creates a visible comment by default", () => {
      const comment = createComment(lessonId, base.user.id, "Hello!");
      expect(comment.hidden).toBe(false);
    });

    it("returns the new comment with correct fields", () => {
      const comment = createComment(lessonId, base.user.id, "Test body");
  ...
  describe("getCommentsForLesson", () => {
    ...
    it("returns only visible comments when includeHidden is false", () => {
      createComment(lessonId, base.user.id, "Visible");
      const hidden = createComment(lessonId, base.user.id, "Hidden");
    ...
    it("returns comments ordered by createdAt ascending", () => {
      const c1 = createComment(lessonId, base.user.id, "First");
      const c2 = createComment(lessonId, base.user.id, "Second");
  ...
  describe("getCommentById", () => {
    ...
    it("returns the comment when it exists", () => {
      const created = createComment(lessonId, base.user.id, "Test");
  ...
  describe("setCommentVisibility", () => {
    it("hides a visible comment", () => {
      const comment = createComment(lessonId, base.user.id, "Test");
  ...
  describe("deleteComment", () => {
    ...
    it("has no effect on other comments", () => {
      const keep = createComment(lessonId, base.user.id, "Keep");
      const del = createComment(lessonId, base.user.id, "Delete");
