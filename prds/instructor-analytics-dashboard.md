...
All metrics are filterable by time range: last 30 days, last 90 days, or all time (driven by a `?range=` URL search param).
...
4. As an instructor, I want to filter all overview metrics by last 30 days, last 90 days, or all time, so that I can track trends over different periods.
...
16. As an instructor, I want to filter all per-course analytics metrics by last 30 days, last 90 days, or all time, so that I can isolate performance in specific windows.
...
- **`/instructor` route update** — Add overview analytics section above the course list. Fetch via server loader using `getInstructorOverview`. Include a time-range selector (30d / 90d / all time) that resubmits the loader via `?range=` search param.

- **`/instructor/:courseId/analytics` route (new)** — Per-course analytics page. Single server loader call. Renders: stat cards, revenue chart + transaction table, enrollment chart, completion rate stat, module quiz pass rate table, lesson drop-off funnel chart. Add an "Analytics" link to the per-course instructor navigation alongside the existing tabs.
...
- Time range filtering uses `from`/`to` Date parameters in service functions; the route layer converts `?range=30d|90d|all` to concrete dates before calling services.
...
- `?range=` search param makes analytics views shareable and bookmarkable.
